...
export default function PanelExperience() {
    ...
    const getData = (limit = 15) => {
        PrivateApi.get('/experience?page='+paginationActual+'&limit='+limit).then((result)=>{
            let newData = [];
    ...
    const deleteData = (id) => {
        if( confirm('Tem certeza que deseja deletar esse item?') ) {
            PrivateApi.delete('/experience/'+id,{
                headers: {
    ...
    useEffect(()=>{
        ...
        Sortable.create(sortableParent,{
            ...
            onEnd: () => {
                ...
                PrivateApi.put('/experience/reorder/',{'order':newOrder},{
                    headers: {
                        'authorization': auth.token
                    }
                })
                                                ...
                                                <tr id={item.id} key={i}>
                                                    ...
                                                    <td>
                                                        <div className="tabled-actions float-right">
                                                            <a title="Editar" href={`/control/experience/edit/${item.id}`}><i className="lni lni-pencil"></i></a>
